...
export default {
 ...
 run: async (client, interaction, guildSettings) => {
  ...
  if (subCommand === "add") {
   ...
   if (role.tags?.botId || role.id === interaction.guild.id) {
    return client.errorMessages.createSlashError(interaction, "You can't add a bot or everyone role!");
   }
   ...
   if (interaction.member.roles.highest.comparePositionTo(role) <= 0) {
    return client.errorMessages.createSlashError(interaction, "You can't add a role which is higher or equal than your highest role!");
   }
   ...
   if (role.tags?.botId || role.id === interaction.guild.id) {
    return client.errorMessages.createSlashError(interaction, "You can't remove a bot or everyone role!");
   }
