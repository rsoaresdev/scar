...
import { shortenText } from "@scar/util/functions/util";
...
export async function guildMemberAdd(client, member) {
 ...
 const autoRoleEnabled = await prismaClient.guild.findFirst({
  where: {
   guildId: member.guild.id,
  },
 });

 if (autoRoleEnabled && autoRoleEnabled.autoRole && autoRoleEnabled.autoRole !== "") {
  //? AutoRole enabled
  if (!member.guild.members.me.permissions.has(PermissionsBitField.Flags.ManageRoles)) return;

  const joinRole = member.guild.roles.cache.get(autoRoleEnabled.autoRole);

  if (joinRole.comparePositionTo(member.guild.members.me.roles.highest) >= 0) return;

  //? If the role exists, add the role to the member
  if (joinRole && !member.roles.cache.has(joinRole.id)) {
   member.roles.add(joinRole.id);
  } else {
   //? If the role does not exist, disable auto-role field
