...
import { createAutoModRule, syncAutoModRule } from "@scar/util/database";
...
export async function enableAntiMention(client, interaction, guildSettings) {
 ...
 if (createdRule) {
  ...
  const ruleToCreate = {
   name: "Disallow mention spam [Scar]",
   creatorId: client.id,
   enabled: true,
   ...
   actions: [
    {
     ...
     metadata: {
      ...
      customMessage: "Message blocked due to containing too many mentions. Rule added by Scar",
     },
    },
   ],
  ...
  if (logChannel) {
   ...
   ruleToCreate.actions.push({
    ...
    metadata: {
     channel: logChannel,
     message: "Message blocked due to containing too many mentions. Rule added by Scar",
    },
   });
  }
