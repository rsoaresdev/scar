...
import { createHTTPAutomodRule, validateAutoModIgnores, validateAutoModRuleActions } from "@scar/util/functions/automod";
...
export async function POST(request) {
 try {
   ...
   data.actions = [
    {
     ...
     metadata: {
      custom_message: "Message blocked due to containing an invite link. Rule added by Scar",
     },
    },
   ];
  }
  ...
  const validatedActions = await validateAutoModRuleActions(data.actions, allChannels, "Message blocked due to containing an invite link. Rule added by Scar");
