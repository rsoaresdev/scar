...
import { AutoRole } from "@/components/client/settings/automod/AutoRole";
...
async function syncAutoRoleRule(guildId) {
 const createdRule = await prismaClient.guild.findFirst({
  where: {
   guildId: guildId,
  },
 });

 if (createdRule && createdRule.autoRole) {
  return true;
 } else {
  return false;
 }
...
export default async function AutomodPage({ params }) {
 ...
 const enabledAutoRole = (await syncAutoRoleRule(serverDownload.id)) || false;
 ...
 return (
  <>
   ...
   <Block className="mb-4">
    {enabledAutoRole ? ( // prettier
     <AutoRole serverId={serverDownload.id} enabled={enabledAutoRole || false} allRoles={allRoles} />
    ) : (
     <AutoRole serverId={serverDownload.id} enabled={false} allRoles={allRoles} />
    )}
   </Block>
   <Block className="mb-4">
    <Header2>
