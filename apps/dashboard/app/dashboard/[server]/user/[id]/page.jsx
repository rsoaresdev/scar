...
import { getFlags } from "@scar/util/functions/user";
import { formatNumber } from "@scar/util/functions/util";
...
export default async function UserPage({ params }) {
 ...
 const userReputation = user.reputation.reduce((a, b) => a + (b["reputation"] || 0), 0);
 ...
 return (
  <>
     ...
     <div className="bg-background-menu-button/70 m-[8px_16px_16px] rounded-lg border border-neutral-800 p-4">
      <div className="flex flex-col items-center justify-center gap-4 sm:flex-row sm:justify-start">
       ...
       <Tooltip content="Total reputation given by other users">
        <div className="flex cursor-help items-center">
         <div className="mr-2 h-3 min-h-3 w-3 min-w-3 rounded-full bg-[#81848f]" />
         {formatNumber(userReputation || 0)} Reputation
        </div>
       </Tooltip>
   ...
   <Block className="mt-4">
    ...
    <ChangeUserReputation userId={user.discordId} username={user.globalName || user.username || "No username"} guildId={serverDownload.id} userReputation={userReputation} />
   </Block>
