...
import { getServer, getGuildPreview, getGuildMember } from "@scar/util/functions/guild";
import { fillMissingDates } from "@scar/util/functions/util";
     ...
     <Block>
      ...
      {/* {guild.publicPage ? (
       <ButtonSecondary href={`/server/${guild.vanity || serverDownload.id}`} className="mx-auto !flex flex-row whitespace-nowrap sm:ml-auto sm:mr-0" target="_blank" rel="noreferrer noopener">
        <ArrowTopRightOnSquareIcon className="mr-2 h-5 min-h-5 w-5 min-w-5" aria-hidden="true" role="img" />
        Server page
       </ButtonSecondary>
      ) : (
       <span className="mx-auto whitespace-nowrap sm:ml-auto sm:mr-0">Powered by Scar</span>
      )} */}
     </Block>
     <Block>
