...
export default {
 name: "contact",
 description: "📝 Contact the Scar team",
 ...
 run: async (client, interaction, guildSettings) => {
  try {
   ...
   const embed = new EmbedBuilder()
    .setDescription(`Click the button below or [click here](${client.config.url}/contact) to contact the Scar team.\n\n>>> **Useful links:**\n- [View all Scar commands](${client.config.url}/commands)\n- [Scar support server](${client.config.url}/support)`)
    .setFooter({
     text: `Requested by ${interaction.member.user.globalName || interaction.member.user.username}`,
     iconURL: interaction.member.user.displayAvatarURL({
      size: 256,
