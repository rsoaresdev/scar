...
async function processReaction(data, message, member) {
 ...
 if (guild.members.me.permissions.has(PermissionsBitField.Flags.ManageRoles)) {
  ...
  guild.members.cache.get(member.id).roles.remove(...userRoles);
 }
}

export async function messageReactionRemove(client, messageReaction, user) {
 try {
  ...
  Logger("error", `[messageReactionRemove]: ${err}`);
 }
}
